...
function App() {
  ...
  if (!token) return <main className="login-page"><section className="login-intro"><div className="login-brand"><span className="brand-mark">F</span><span>Fleet<span>Platform</span></span></div><p className="eyebrow">Command every journey</p><h2>Your fleet.<br />In perfect motion.</h2><p>Live visibility, safer drivers, and smarter operations—all from one focused workspace.</p><div className="login-pulse"><i /> Live operations overview</div></section><form className="login-card" onSubmit={login}><div className="mobile-login-brand"><span className="brand-mark">F</span></div><p className="eyebrow">Secure workspace</p><h1>Welcome back</h1><p>Enter your details to access your fleet.</p>{error && <div className="alert">{error}</div>}<label>Username<input name="username" required autoComplete="username" placeholder="Your username" /></label><label>Password<input name="password" required type="password" autoComplete="current-password" placeholder="Your password" /></label><button className="primary-button">Sign in <span aria-hidden="true">→</span></button><small className="login-footnote">Protected fleet management access</small></form></main>
...
/* Previous map retained temporarily for visual regression reference.
function LegacyLiveMap({ positions, onRefresh }: { positions: Position[]; onRefresh: () => void }) {
...
function LiveMap({ positions, onRefresh }: { positions: Position[]; onRefresh: () => void }) {
  ...
  const reporting = livePositions.filter((item) => item.position)
  const visible = livePositions.filter((item) => item.registrationNo.toLowerCase().includes(query.trim().toLowerCase()))
  const selected = livePositions.find((item) => item.vehicleId === selectedId) ?? null
  const moving = reporting.filter((item) => item.position!.speed > 1).length
  const idle = reporting.length - moving
  const centre: [number, number] = reporting.length ? [reporting[0].position!.latitude, reporting[0].position!.longitude] : [23.8103, 90.4125]
  const viewportPositions = selected?.position ? [selected] : reporting
  return <section className="tracking-console">
    <aside className="fleet-rail">
      <div className="fleet-rail-head"><div><span className="rail-kicker">Live fleet</span><strong>{livePositions.length} vehicles</strong></div><button className="map-refresh" onClick={onRefresh} aria-label="Refresh positions">&#8635;</button></div>
      <div className="fleet-search"><span aria-hidden="true">&#8981;</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search vehicle" aria-label="Search vehicle" /></div>
      <div className="fleet-filters"><span><i className="dot moving" />{moving} moving</span><span><i className="dot idle" />{idle} idle</span><span><i className="dot offline" />{livePositions.length - reporting.length} offline</span></div>
      <div className="vehicle-feed">{visible.length ? visible.map((item) => {
        const state = !item.position ? 'offline' : item.position.speed > 1 ? 'moving' : 'idle'
        return <button key={item.vehicleId} className={`vehicle-row ${selected?.vehicleId === item.vehicleId ? 'selected' : ''}`} onClick={() => setSelectedId(item.vehicleId)}>
          <span className={`vehicle-symbol ${state}`} aria-hidden="true">&#9650;</span><span className="vehicle-copy"><strong>{item.registrationNo}</strong><small>{item.position ? `Updated ${new Date(item.position.fixTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'No GPS position'}</small></span><span className="vehicle-speed"><strong>{item.position ? Math.round(item.position.speed) : '—'}</strong><small>kn</small></span>
        </button>
      }) : <p className="rail-empty">No matching vehicles</p>}</div>
    </aside>
    <div className="map-stage">
      <div className="map-statusbar"><div><span className={`socket-state ${socketState}`}>{socketState === 'live' ? 'Live feed' : socketState === 'connecting' ? 'Connecting…' : 'Offline'}</span><strong>{selected ? selected.registrationNo : 'Fleet overview'}</strong></div><span className="last-sync">Auto-updating positions</span></div>
      <MapContainer className="live-map" center={centre} zoom={reporting.length ? 13 : 10} scrollWheelZoom><TileLayer attribution="&copy; OpenStreetMap contributors" url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" /><MapViewport positions={viewportPositions} />{reporting.map((item) => { const isSelected = selected?.vehicleId === item.vehicleId; const isMoving = item.position!.speed > 1; return <CircleMarker eventHandlers={{ click: () => setSelectedId(item.vehicleId) }} key={item.vehicleId} center={[item.position!.latitude, item.position!.longitude]} radius={isSelected ? 13 : 9} pathOptions={{ color: '#ffffff', weight: 3, fillColor: isMoving ? '#08a88a' : '#f59e0b', fillOpacity: 1 }}><Popup><div className="vehicle-popup"><span>{isMoving ? 'Moving' : 'Idle'}</span><strong>{item.registrationNo}</strong><b>{item.position!.speed.toFixed(1)} kn</b><small>Updated {new Date(item.position!.fixTime).toLocaleString()}</small></div></Popup></CircleMarker>})}</MapContainer>
      <div className="map-legend"><span><i className="dot moving" />Moving</span><span><i className="dot idle" />Idle</span><span><i className="dot offline" />No position</span></div>
      {selected && <div className="vehicle-detail"><button onClick={() => setSelectedId(null)} aria-label="Close vehicle details">&times;</button><p>Selected vehicle</p><strong>{selected.registrationNo}</strong>{selected.position ? <div><span><b>{selected.position.speed.toFixed(1)}</b> knots</span><span><b>{selected.position.latitude.toFixed(5)}</b> latitude</span><span><b>{selected.position.longitude.toFixed(5)}</b> longitude</span></div> : <small>No current GPS position</small>}</div>}
    </div>
  </section>
}
